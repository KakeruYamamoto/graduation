...
class Event < ApplicationRecord
  ...
  def event_end_check
    if e_date_end.nil? || e_date_end < e_date_start
      errors.add(:e_date_end, 'がイベント開始日時より後になっています')
      end
  end

  def event_date_check
    if e_date_start <= Time.now
      errors.add(:e_date_start, 'が現在の日時より下回っています。”当日開催はご遠慮ください”')
    end
  end
end
